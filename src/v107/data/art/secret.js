...
const secret = {
  ...
  25: {
    ...
    effect: [
      '攻击时有50%概率累积1次逍遥御风，被攻击必定累积1次逍遥御风',
      '当累积点数达到9点时触发：',
      '└从下次开始受到攻击开始计算，闪避15时序并清空封穴',
      '└直到人物下次行动前，免疫封穴，被攻击后集气值不会低于0',
      '└人物下次行动时累积归零，可以重新累积逍遥御风',
    ],
    type: 1,
  },
  26: {
  ...
  34: {
    ...
    effect: [
      '攻击命中敌人时30%概率种下金蚕盅，目标下次行动前发作一次，扣除其最大生命5%，并有50%概率乱心、放弃本次攻击',
      '金蚕会吞食目标身上的灵蛇蛊、玉蟾蛊、风蜈蛊、天蛛蛊、圣蝎蛊，每吞一种增加3%最大生命伤害，每种只成长一次，最多五层（总伤害20%）。玉蟾先吞增益，再喂金蚕；被吞五蛊立即清除',
      '重复种下不叠加多只、不重置成长，发作后金蚕及成长全部清除，可再次种下',
    ],
    type: 1,
  },
  35: {
  ...
  42: {
    ...
    effect: [
      ...
      '使用五岳剑法攻击概率发动气贯五岳提升1000气攻，暴怒必定发动，主运五岳剑诀必破气防',
    ],
    type: 1,
  },
  43: {
  ...
  45: {
    ...
    effect: [
      `易筋锻骨：主运${getArtName(85)}，40%概率最高优先级免疫杀气，免疫吸内、吸血、吸体、吸怒`,
  ...
  73: {
    ...
    effect: [
      '气防翻倍，自身破绽时受到攻击免疫伤害，且集气位置+200，每回合最多触发3次',
    ],
    type: 1,
  },
  74: {
  ...
  101: {
    ...
    cheat: {649: true},
    effect: [
      '修炼青元剑诀、梵圣真魔功、青竹蜂云剑法后：攻击必定触发【辟邪神雷】——使目标散功并陷入天地失色20时序（期间天赋与词条失效），结束时对目标周围三格敌人各造成1000真实伤害',
      '触发涅槃重生时进入【涅槃法相】：暴怒并进入金刚法相、走火入魔、霸体三回合，期间攻击追加目标生命最大值10%伤害并造成撕裂、震慑',
      '战斗中习得绝技【傀儡术】：尚无傀儡时可标记一名敌人，本场将其击败即炼成傀儡',
      '已持有傀儡时改为召唤：傀儡以我方身份参战，自动行动，不能吃药、不可被治疗',
      '傀儡只能召唤主角当前等级档（三流/二流/一流/豪侠/宗师/神话），升档后旧傀儡失效需重选',
    ],
    type: 1,
  },
  102: {
    id: 102,
    name: '七星聚会',
    cheat: {122: true, 368: true, 441: true, 443: true},
    effect: [
      '以全真武功攻击时35%概率触发【七星聚会】：',
      '追加600气攻，集气条前进100，并随机借得一至七颗星之力（可多种叠加，随机触发）',
      '天枢贪狼·杀伐：追加700气攻',
      '天璇巨门·蚀骨：目标附加内伤15点，并减攻20%持续两回合',
      '天玑禄存·养元：回复自身8%生命与300内力',
      '天权文曲·巧变：追加一次连击，且本次攻击必中破绽',
      '玉衡廉贞·封禁：高概率封穴，目标集气倒退200',
      '开阳武曲·刚武：必定暴击，伤害+30%',
      '瑶光破军·破阵：无视40%防御，追加300杀气',
    ],
    type: 1,
  },
  103: {
    ...
    cheat: {761: true},
    effect: [
      '消耗：30体力，1000内力。CD：30时序',
      '使用后获得九天剑气状态（持续20时序）',
      '└攻击时向武功落点方向发出9道剑气，每道造成御剑能力25%的伤害，穿透直线上所有敌人',
      '└剑气命中后30%概率附加创伤（每时序损失 1% 点生命、禁疗持续5时序）',
      '└万剑归宗剑气满100时，九天剑气伤害+50%，道数+3（共 12 道）',
    ],
    type: 1,
  },
  ruLaiShenZhang: {
    ...
    cheat: {88: true},
    effect: [
      `${getArtName(88)}攻击时概率触发：`,
      `目标生命值低于50%触发概率为30%，不低于50%触发概率为10%，主运${getArtName(85)}额外增加20%概率，通臂拳奇穴增加10%概率`,
      '目标生命值低于10%时，有(目标损失生命值÷目标最大生命值)×30%固定概率（通臂拳奇穴额外增加10%概率）变为如来神掌·灭，直接斩杀目标',
      '没有触发如来神掌·灭，则追加1200气攻，攻击无误伤，附加目标最大生命值8%的真伤',
    ],
    type: 2,
  },
  miZongAoYi: {
  ...
  xiangLongJiYi: {
    ...
    cheat: {86: true},
    effect: [
      `${getArtName(86)}攻击时必定触发两种极意之一：`,
      '极意1：攻击必定三连。15%概率触发，天赋狂龙天征增加40%概率；加入丐帮增加30%概率；奇穴通臂拳增加15%概率；装备音响满级增加25%概率且有概率四连；暴怒必定触发',
      '极意2：攻击变为大范围攻击，气攻+1000。没触发极意1时必定触发',
    ],
    type: 2,
  },
  xuanMingJiYi: {
  ...
  fuShiDuJiYi: {
    ...
    effect: [
      ...
      '攻击的气攻+1000，攻击无误伤，引爆中毒扣除目标中毒点数的生命值',
    ],
    type: 2,
  },
  jinGangFuMoZhen: {
    ...
    cheat: {171: true},
    effect: [
      `金刚伏魔圈护体：2人学会${getArtName(171)}，气防+500`,
      `真·金刚伏魔圈护体： 2人以上学会${getArtName(171)}，气防+1000`,
    ],
    type: 2,
  },
};

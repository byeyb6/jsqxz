...
export function lingXu(lv) {
  return {
    ...
    name: `凌虚 Lv${lv}`,
    effect: [
      '战斗开始每10时序自动获得「凌虚」状态3时序：80%概率免疫反弹、反击、反震；免疫减速、锁足；受到攻击概率打断对方并进行反击',
    ],
  };
}
...
export function hunDunTaiXuan(lv) {
  return {
    ...
    effect: [
      `受到伤害时触发：`,
      `└五岳倒为轻：${(lv + 1) * 6}%概率触发，将所受伤害的${lv * 30}%转化为生命恢复（满血或者当前生命值低于所受伤害，不会触发）`,
      `└救赵挥金锤：${(lv + 1) * 6}%概率触发，打断敌方攻击，并使敌方全体集气位置减${lv * 50}点`,
    ],
  };
}
...
export function xueMo(lv) {
  return {
    ...
    name: `血魔 Lv${lv}`,
    effect: [
      `攻击时造成伤害时${lv * 25}%概率将伤害的${lv * 10}%转化为恢复自身生命，最多恢复身最大生命值的5%，激活奇穴修罗道后溢出部分转化为护盾`,
    ],
  };
}
...
export function buQu(lv) {
  return {
    id: 1019,
    name: `不屈 Lv${lv}`,
    effect: [`集气位置小于0时，额外减少受到的伤害${lv * 20}%`],
  };
}
...
export function liuDaoLunHui(lv) {
  return {
    ...
    name: `六道轮回 Lv${lv}`,
    effect: [
      `进入战斗开始降低${lv * 4 + 4}%附加三围，每回合+${lv * 4 + 4}%，每六个回合一轮回，轮回结束时六道轮回附加的三围变为0，并恢复${lv * 7.5}%的生命、内力、体力，清除所有异常状态`,
    ],
  };
}
...
export function xuanZhiTaiYi(lv) {
  return {
    ...
    effect: [
      ...
      `（第九词条需激活：侠客行正线侠客岛战胜李白，邪线侠客岛战胜石破天可激活）`,
    ],
  };
}
...
export function taiYanShiFa(lv) {
  return {
    ...
    effect: [
      ...
      `（第九词条需激活：挑战张三丰，张三丰血量降低50%或坚持200时序后可激活）`,
    ],
  };
}
...
export function youDingSiGu(lv) {
  return {
    ...
    effect: [
      ...
      '└冰封时激发【玄冰蛊】：受到的伤害大于生命最大值20%时冻结自己5时序，且冻结期间免疫伤害；如果受到伤害时自身已经处于冻结状态则不会触发【玄冰蛊】；CD99时序',
...
export function jianQiNiQianKun(lv) {
  return {
    id: 1061,
    name: `剑气逆乾坤 Lv${lv}`,
    effect: [
      '自身血量低于30%，受到攻击时消耗复活次数（已复活无法触发），使敌方全体集气位置归0，并发动【剑气逆乾坤】：',
      '使用剑法攻击免疫反击和反伤，附加伤害20%吸血（上限100×外功层级），追加1000气攻；五岳神剑攻击必破气防；受到攻击时减少(御剑系数÷2)的真伤（上限300）；持续整场战斗，切换不带该词条的内功会暂时失效',
      `（第九词条需激活：笑傲正线思过崖华山秘洞先调查墙壁获得五岳剑诀，再调查骷髅挑战风清扬可激活）`,
    ],
  };
}
...
export function guJingHuShen(lv) {
  return {
    ...
    name: `蛊经护身 Lv${lv}`,
    effect: [
      `冰封且未冻结时，单次受到的伤害大于生命最大值40%，触发【玄冰盅】：冻结自身3时序，期间免疫伤害`,
      `受到攻击时若自身灼烧且未停止行动，触发【火蝶盅】：点燃3格内敌方3时序，冷却60时序`,
    ],
  };
}
